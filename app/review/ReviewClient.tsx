...
export function ReviewClient() {
  ...
  function answerQuestion(question: TakkenWrongQuestion, choiceNumber: number) {
    ...
    if (isCorrect) {
      ...
      setWrongQuestions((questions) => questions.filter((wrongQuestion) => wrongQuestion.id !== question.id));
      return;
    }
  ...
  if (isReviewing && isFinished) {
    ...
    return (
      <article className="container practice-page review-page">
        <section className="card practice-result" aria-labelledby="review-result-heading">
          ...
          <div className="action-row">
            {remainingCount > 0 ? (
              <button className="button button-primary" type="button" onClick={startReview}>
                残りをもう一度復習する
              </button>
            ) : null}
            <button className="button button-secondary" type="button" onClick={returnToList}>
              復習一覧へ戻る
            </button>
          </div>
        </section>
  ...
  if (isReviewing && currentQuestion) {
    ...
    return (
      <article className="container practice-page review-page">
        ...
        <section className="card practice-question-card" aria-labelledby="review-question-heading">
          <div className="practice-meta-row">
            <span>{currentQuestion.subject}</span>
            <span>{currentQuestion.topic}</span>
            {currentQuestion.is_exemption_question ? <span>登録講習免除対象問</span> : null}
          </div>
  ...
  return (
    <article className="container practice-page review-page">
      <div className="practice-hero">
        ...
        <span className="status-badge">保存件数：{isLoaded ? wrongQuestions.length : "確認中"}件</span>
      </div>

      <section className="section-block" aria-labelledby="review-list-heading">
        <div className="section-heading compact-heading">
          <p className="eyebrow">Saved questions</p>
          <h2 id="review-list-heading">間違えた問題一覧</h2>
          <p className="review-storage-note">localStorage key：{TAKKEN_WRONG_QUESTIONS_STORAGE_KEY}</p>
        </div>
        ...
        {wrongQuestions.length > 0 ? (
          <>
            <div className="action-row review-actions">
              ...
              <button className="button button-secondary" type="button" onClick={clearSavedQuestions}>
                保存した間違いをすべて削除
              </button>
            </div>
            ...
            <div className="review-question-grid">
              {wrongQuestions.map((question) => (
                <section className="card review-question-card" key={question.id}>
                  ...
                  <div className="practice-meta-row">
                    ...
                    {question.is_exemption_question ? <span>登録講習免除対象問</span> : null}
                  </div>
          ...
          <section className="card review-empty-card">
            ...
            <p>
              /practice の年度別演習で不正解だった問題があると、ここに一覧表示されます。
